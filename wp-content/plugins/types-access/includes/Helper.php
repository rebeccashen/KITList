	...
	*/
	public static function wpcf_access_add_editor_icon( ){
		...
		$out .= '<div class="editor_addon_dropdown js-wpcf-access-editor-popup" id="editor_addon_dropdown_access_' . rand() . '">
                <h3 class="title">' . __('Insert conditionaly-displayed text', 'wpcf_access') . '</h3>
                <div class="close">&nbsp;</div>
                <div class="editor_addon_dropdown_content">';
        
		$roles = $wp_roles->roles;
		$out .= '<h3>'.__('Select roles: ', 'wpcf_access').'</h3>';
		...
		$out .= '<label>
        		<input type="checkbox" class="js-wpcf-access-list-roles" value="Guest" /> '.__('Guest', 'wpcf_access').'</label><br>';	
		$out .= '</p>';
		...
		$out .= '<p class="wpcf-access-margin">
			<label>
        		<input type="radio" class="js-wpcf-access-shortcode-operator" name="wpcf-access-shortcode-operator" value="allow" /> '. __('Only users belonging to these roles will see the text', 'wpcf_access') . '</label><br>
        	<label>
        		<input type="radio" class="js-wpcf-access-shortcode-operator" name="wpcf-access-shortcode-operator" value="deny" /> '. __('Everyone except these roles will see the text', 'wpcf_access') . '</label><br>
		</p>';
        $out .= '        
        </div>
	...
	public static function wpcf_access_select_group_metabox_files( ) 
    {
		...
		if ( isset($post) && is_object($post) && $post->ID != '' ){
			...
			if ( !wp_script_is('toolset-colorbox', 'registered')  ){
				wp_register_script('toolset-colorbox', TACCESS_ASSETS_URL.'/js/popup/jquery.colorbox-min.js', array('jquery'), WPCF_ACCESS_VERSION, false);
			}
			if ( !wp_script_is('toolset-colorbox', 'enqueued')  ){
				 wp_enqueue_script('toolset-colorbox');
			}
			if ( !wp_script_is('views-utils-script', 'registered')  ){
				wp_register_script('views-utils-script', TACCESS_ASSETS_URL.'/js/popup/utils.js', array('jquery', 'wp-pointer'), WPCF_ACCESS_VERSION, false);
			}
			...
			if ( !wp_script_is('icl_editor-script', 'registered')  ){
				wp_register_script('icl_editor-script', TACCESS_ASSETS_URL.'/common-visual-editor/js/icl_editor_addon_plugin.js', array(), WPCF_ACCESS_VERSION, false);
			}
			...
			if ( !wp_style_is('editor_addon_menu', 'registered')  ){
				wp_register_style('editor_addon_menu', TACCESS_ASSETS_URL.'/common-visual-editor/css/pro_dropdown_2.css', '', WPCF_ACCESS_VERSION);
			}
			...
			if ( !wp_style_is('editor_addon_menu_scroll', 'registered')  ){
				wp_register_style('editor_addon_menu_scroll-css', TACCESS_ASSETS_URL.'/common-visual-editor/css/scroll.css', '', WPCF_ACCESS_VERSION);
			}
			if ( !wp_style_is('editor_addon_menu_scroll-css', 'enqueued')  ){
				 wp_enqueue_style('editor_addon_menu_scroll');
			}
			
			
			if ( !wp_style_is('toolset-colorbox', 'registered')  ){
				wp_register_style('toolset-colorbox', TACCESS_ASSETS_URL.'/css/colorbox.css', '', WPCF_ACCESS_VERSION);
			}
			...
			if ( !wp_style_is('notifications', 'registered')  ){
				wp_register_style('notifications', TACCESS_ASSETS_URL.'/css/notifications.css', '', WPCF_ACCESS_VERSION);
			}
			if ( !wp_style_is('wpcf-access-dev', 'registered')  ){
				wp_register_style('wpcf-access-dev', TACCESS_ASSETS_URL.'/css/basic.css', '', WPCF_ACCESS_VERSION);
			}
			if ( !wp_style_is('notifications', 'enqueued')  ){
				 wp_enqueue_style('notifications');
			}
			
			if ( !wp_style_is('views-dialogs-css', 'registered')  ){
				wp_register_style('views-dialogs-css', TACCESS_ASSETS_URL.'/css/dialogs.css', '', WPCF_ACCESS_VERSION);
			}
			if ( !wp_style_is('views-dialogs-css', 'enqueued')  ){
...
//function wpcf_access_role_has_cap_filter($capabilities, $cap, $role) {}
            ...
            'types_access' => array(
                ...
                'subpages' => array(
                    'types_access_settings' => array(
                        'title' => __('Import/Export', 'wpcf_access'),
                        'menu' => __('Import/Export', 'wpcf_access'),
                    ...
                    'types_access_help' => array(
                        'title' => __('Help', 'wpcf_access'),
                        'menu' => __('Help', 'wpcf_access'),
                        'function' => array('Access_Helper', 'wpcf_access_admin_help_page'),
                        'subpages' => array(
                            'types_access_debug' => array(
                                'title' => __('Debug information', 'wpcf_access'),
                                'menu' => __('Debug information', 'wpcf_access'),
    ...
    public static function wpcf_access_admin_menu_load() 
    {
		...
		TAccess_Loader::loadAsset('STYLE/views-dialogs-css', 'views-dialogs-css');
    ...
    public static function wpcf_access_types_caps_predefined() 
    {
        $modes = array(
            ...
            'read_private' => array(
                'title' => __('Preview any', 'wpcf_access'),
                'role' => 'administrator',
