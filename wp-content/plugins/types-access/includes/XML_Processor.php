...
final class Access_XML_Processor
{
    ...
    private static function toXml($array, $root_element)
    {
        ...
        $xml .="</$root_element>";//print $xml;exit;
        return $xml;
    }
	...
	private static function ArraytoSettings($data) 
    {
		...
		foreach ($data as $data_key => $data_value){
			...
			//Custom roles
			if ( $data_key == 'access_custom_roles' && is_array($data_value['item']) && count($data_value['item']) > 0 ){
        ...
        if ($data3)
        {
            ...
            foreach ($data['third_party'] as $area=>$adata)
            {
                ...
                if (isset($access_settings['third_party'][$area]))
                {
					...
					//print_r($intersect['third_party'][$area]);
					//print_r($access_settings['third_party'][$area]);exit;
                }
                else
                {
