...
public static function wpcf_access_admin_edit_access($enabled = true)
{
    ...
    if (!empty($types))
    {
        $output .= '<h3 class="wpcf-section-header">' . __('Post Types', 'wpcf_access') . '</h3>';
        foreach ($types as $type_slug => $type_data) {
			...
			$shortcuts[__('Post Types', 'wpcf_access')][] = array(
            ...
            if ((empty($type_data['_wpcf_access_outsider'])
                ...
                $output .= '>' . __('This post type will inherit the same access rights as the standard WordPress Post when not Managed by Access.', 'wpcf_access');
				$output .= '</div>';
            }
			...
			$the_query = new WP_Query( $args );
				...
				if ( $the_query->found_posts > 0 ) {
					...
					$message = sprintf(
							__( 'Some %s may have different read access settings because they belong to these access groups: %s', 'wpcf_access' ),
				...
				if ( $type_slug == 'attachment' ){
					...
					__( 'This section controls access to media-element pages and not to media that is included in posts and pages.', 'wpcf_access' )
					. '</div>';
				}
    ...
    if (!empty($taxonomies))
    {
        $output .= '<h3 class="wpcf-section-header">' . __('Taxonomies', 'wpcf_access') . '</h3>';
        foreach ($taxonomies as $tax_slug => $tax_data)
        {
            $mode = 'not_managed';
        ...
        /*if ('product_shipping_class'==$tax_slug)
        {
            ...
            $shortcuts[__('Taxonomies', 'wpcf_access')][] = array($tax_data['labels']['name'], $tax_slug);
    ...
    foreach ($areas as $area)
    {
        ...
        foreach ($groups as $group)
        {
            ...
            $output .= '<div class="wpcf-access-type-item js-wpcf-access-type-item">';
				...
				$output .= '</div>	<!-- .wpcf-access-mode -->';
				$output .= '<p class="wpcf-access-top-anchor"><a href="#wpcf-access-top-anchor">'. __('Back to Top', 'wpcf_access') .'</a></p>';	//3rd party
            $output .= '</div>	<!-- .wpcf-access-type-item -->';
        }
    }
	...
	foreach ( $settings_access as $group_slug => $group_data) {
			...
			if ( $show_section_header ){
				$output .= '<h3 class="wpcf-section-header">' . __('Custom groups', 'wpcf_access') . '</h3>';
            ...
            $output .= '<a name="' . $group['id'] . '"></a>';
            $output .= '<div class="wpcf-access-custom-group wpcf-access-type-item is-enabled js-wpcf-access-type-item" id="js-box-' . $group_slug . '">';
				$output .= '<h4>' . $group['name'] . '</h4>';
				$output .= '<div class="wpcf-access-mode">';
					$caps = array();
					$saved_data = array();
					...
					$output .= '<p class="wpcf-access-buttons-wrap">';
						...
						$output .= '<input data-group="' . $group_slug . '" type="button" value="' . __('Modify Group', 'wpcf_access') . '"  class="js-wpcf-modify-group button-secondary" /> ';
						$output .= '<input data-group="' . $group_slug . '"  type="button" value="' . __('Remove Group', 'wpcf_access') . '"  class="js-wpcf-remove-group button-secondary" /> ';
						$output .= self::wpcf_access_submit_button($enabled, true, $group['name']);
					$output .= '</p>';
...
//	$output .= '<div class="wpcf-access-new-button-wrap">';
		$output .= '<button data-label="'.__('Add Group','wpcf_access').'" value="'. __('Add custom group', 'wpcf_access') .'" class="button button-large button-secondary wpcf-add-new-access-group js-wpcf-add-new-access-group"><i class="icon-plus"></i>'. __('Add custom group', 'wpcf_access') .'</button>';
//		$output .= '<p class="wpcf-access-top-anchor"><a href="#wpcf-access-top-anchor">'. __('Back to Top', 'wpcf_access') .'</a></p>';
//	$output .= '</div>';
	...
	$output .= '<div class="js-submit-button-all"><input type="submit"
		   ...
		   class="wpcf-access-submit button-primary js-wpcf-access-submit"  /><span class="ajax-loading spinner"></span></div>';
    $output .= '</form>';
    $output .= '<p class="wpcf-access-top-anchor anchor-floated"><a href="#wpcf-access-top-anchor">'. __('Back to Top', 'wpcf_access') .'</a></p>';

    $output .= self::wpcf_access_new_role_form($enabled);
    $shortcuts[__('Custom Roles', 'wpcf_access')] = array(array(__('Custom Roles', 'wpcf_access'), "custom-roles"));
    ...
    if (!empty($shortcuts))
    {
        ...
        $shortmenus .= '<h3>' . __('On this page', 'wpcf_access') . '</h3>';
...
public static function wpcf_access_admin_roles_dropdown( $roles, $name, $data = array(), $dummy = false, $enabled = true, $exclude = array() ) {
    ...
    $output .= isset($data['predefined']) ? 'class="wpcf-access-predefied-' . $data['predefined'] . '">' : '>';
...
public static function wpcf_access_reset_button( $type_slug, $type = 'type', $enabled = true, $managed = true ) {
    ...
    if (!$enabled) {
        ...
        $output .= ' href="'
                ...
                . '" onclick="if (confirm(\''
                . addslashes(__('Are you sure? All permission settings for this type will change to their default values.', 'wpcf_access'))
...
public static function wpcf_access_submit_button($enabled = true, $managed = true, $id = '') {
	...
	/*
	 * <input type="submit"
		   value="<?php _e('Save changes', 'wpcf_access'); ?>"
	...
	<input type="submit"
		   value="<?php echo __('Save ', 'wpcf_access').$id; ?>"
		   id="submit-<?php echo $_id; ?>"
...
public static function wpcf_access_admin_set_custom_roles_level_form($roles, $enabled = true)
{
    ...
    if (empty($custom_roles))
    {
        return '<div id="wpcf-access-custom-roles-wrapper">'
                . __('No custom roles defined', 'wpcf_access') .
				'</div>';
    }


    $output .= '<div id="wpcf-access-custom-roles-wrapper">';
	$output .= '<p>' . __('The user level determines which actions can be performed by each user in the WordPress admin.', 'wpcf_access') . '</p>';
						...
						if (isset($builtin_roles[$index])) {
							...
							foreach ($builtin_roles[$index] as $role => $details) {
								$row .= '<div class="wpcf-access-roles wpcf-access-roles-builtin">';
									...
									$row .= '<span>: : <a href="#" class="wpcf-access-view-caps" data-role="' . sanitize_title($role) . '">' . __('View permissions', 'wpcf_access') . '</a></span>';
								$row .= '</div>';
							}
						}
        if (isset($custom_roles[$index]))
        {
            ...
            foreach ($custom_roles[$index] as $role => $details) {
                ...
                $dropdown .= '<div class="button-group">
								...
								<button ' . 'class="wpcf-access-change-level-apply button button-primary" data-message="' . __('By changing the level, you will reset the capabilities. Are you sure you want to do this?', 'wpcf_access') . '">' . __('Apply', 'wpcf_access') . '</button>
							</div>'
						. '</div>';
                ...
                if ( isset($details['capabilities']['wpcf_access_role']) || ( defined('WP_ACCESS_ADVANCED') && WP_ACCESS_ADVANCED == true ) ) {
	                $row .= '<span>: : <a href="javascript:;" class="wpcf-access-change-level">' . __('Change level', 'wpcf_access') . '</a></span>';
	                $row .= $dropdown;

					//Change Caps link
					$row .= ' <span>: : <a href="#" class="wpcf-access-change-caps" data-role="' . sanitize_title($role) . '">' . __('Change permissions', 'wpcf_access') . '</a></span> ';
	                $row .= ' <span>: : <a ';
	                if ($enabled) {
						...
						$row .= 'href="#" data-href="#wpcf-access-reassign-' . sanitize_title($role) . '" class="wpcf-access-delete-role js-wpcf-access-delete-role"';
	                } else {

	                    $row .= 'href="javascript:;"';
	                }
	                $row .= '>' . __('Delete role', 'wpcf_access') . '</a></span> '
	                        . '';
	                if ($enabled) {
    ...
    if (isset($custom_roles['not_set']))
    {
        ...
        foreach ($custom_roles['not_set'] as $role => $details)
        {
            ...
            $dropdown .= '</select>
							<button class="wpcf-access-change-level-apply button-primary">' . __('Apply', 'wpcf_access') . '</button>
            ...
            $output .= '>' . __('Change level', 'wpcf_access') . '</a></span>'
                    . '';
            if ($enabled) {
                $output .= $dropdown;
            }
...
//                $output .= 'href="#TB_inline?height=155&width=500&inlineId=wpcf-access-reassign-' . $role . '&modal=true" class="wpcf-access-delete-role thickbox"';
                $output .= 'href="#" data-href="#wpcf-access-reassign-' . $role . '" class="wpcf-access-delete-role js-wpcf-access-delete-role"';	// TODO: Gennadi, please check if correct popup displays; I'm not sure how to launch it
            } else {
                $output .= 'href="javascript:;"';
            }
            $output .= '>' . __('Delete role', 'wpcf_access') . '</a></span>'
                    . '';
            if ($enabled) {
...
public static function wpcf_access_permissions_table($roles, $permissions, $settings,
    ...
    $output .= '<table class="wpcf-access-table js-access-table">';
		$output .= '<tr>';
			$output .= '<th>' . __('Action', 'wpcf_access') . '</th>';
			...
			$output .= '<th>' . __('Specific user', 'wpcf_access') . '</th>';

			if ( $group_id == 'types' && $id != 'attachment') {
				$output .= '<th>' . __('When disabled', 'wpcf_access') . '</th>';
			}
		$output .= '</tr>';
    $output .= '<tbody>';
			...
			if ( $permission_slug == 'read'  && $role != 'administrator' && $id != 'attachment' ) {
				...
				if ( isset($current_custom_errors[$role]) && !empty($current_custom_errors[$role]) ) {
					...
					if ( $error_type_value == 'error_404' ) {
						$text = '404';
						$link_title = __('Show 404 - page not found','wpcf_access');
					} elseif ( $error_type_value == 'error_ct' ) {
						$text = __('Template', 'wpcf_access').': '. self::get_content_template_name($error_value_value);
						$link_title = __('Show Content Template','wpcf_access').' - '.self::get_content_template_name($error_value_value);
					} else {
						$text = __('PHP Template', 'wpcf_access').': '. $error_value_value;
						$link_title = __('Show Page template','wpcf_access').' - '. $error_value_value;
					}
				}
				...
				if ( isset($current_archive_custom_errors[$role]) && !empty($current_archive_custom_errors[$role]) && isset($type_data['has_archive']) && $type_data['has_archive'] == 1  ) {
					...
					if ( $archive_error_type_value == 'default_error' ) {
						$archive_text = __('Display: \'No posts found\'','wpcf_access');
					} elseif ( $archive_error_type_value == 'error_ct' ) {
						$archive_text = __('View Archive', 'wpcf_access').': '. self::get_view_name($archive_error_value_value);
					} elseif ( $archive_error_type_value == 'error_php' ) {
						$archive_text = __('PHP Archive', 'wpcf_access').': '. preg_replace("/.*(\/.*\/)/","$1",$archive_error_value_value);
					}else{
						$archive_text = '';	
					}
				...
				if( isset($type_data['has_archive']) && $type_data['has_archive'] == 1 ){
					$is_archive = 1;
					$link_title = ' title="'.__('Set errors','wpcf_access').'" ';
				}else{
					if ( !empty($link_title) ){
					$link_title = ' title="'.__('Set single page error','wpcf_access').'. ('.$link_title.')" ';
					}else{
						$link_title = ' title="'.__('Set single page error','wpcf_access').'" ';
					}	
				}
...
public static function wpcf_access_suggest_user( $enabled = true, $managed = false, $name = '' )
{
    ...
    $output = '<div class="types-suggest-user types-suggest" id="types-suggest-user-' . $_id . '">';
		$output .= '<input type="text" class="input" placeholder="' . esc_attr__('search', 'wpcf_access') . '"';
		if (!$enabled || !$managed) {
			$output .= ' readonly="readonly" disabled="disabled"';
		}
		$output .= ' />';
		$output .= '<img src="' . esc_url(admin_url('images/wpspin_light.gif')) . '" class="img-waiting" alt="" />';
		$output .= '<div class="button-group js-suggest-user-controls"><button class="cancel toggle button button-small button-secondary">' . __('Cancel', 'wpcf_access') . '</button> ';
		$output .= '<button class="confirm toggle button button-small button-primary">' . __('OK', 'wpcf_access') . '</button></div>';
...
public static function wpcf_access_new_role_form($enabled)
{
    ...
    $output .= '<div id="wpcf-access-new-role" class="wpcf-access-new-role-wrap">';
		...
		$output .= '><i class="icon-plus"></i>' . __('New role', 'wpcf_access') . '</button>';

		$output .= '<div class="toggle">';
			$output .= '<input type="text" name="types_access[new_role]" class="input" value="" /> ';
			$output .= '<button class="cancel toggle button-secondary js-cancel">' . __('Cancel', 'wpcf_access') . '</button> ';
			$output .= '<button class="confirm toggle button-primary js-confirm" disabled="disabled">' . __('OK', 'wpcf_access') . '</button> ';
			$output .= '<img src="' . esc_url(admin_url('images/wpspin_light.gif')) . '" class="img-waiting" alt="" />';
		$output .= '</div>';
...
public static function wpcf_access_reassign_role_form($role)
{
	...
	$output .= '<div class="hidden">';
		$output .= '<div class="wpcf-access-reassign-role" id="wpcf-access-reassign-'
				. sanitize_title($role) . '">';
			$output .= '<div class="wpv-dialog">';
				$output .= '<div class="wpv-dialog-header">' . __( 'Delete role', 'wpcf_access' ) . '</div>';
				$output .= '<div class="wpv-dialog-content">';
				...
				$output .= '<div class="ajax-response"></div>';
				$output .= '</div><!-- .wpv-dialog-content -->';

				$output .= '<div class="wpv-dialog-footer">';
				$output .= '<input type="hidden" name="wpcf_access_delete_role" value="' . $role . '" />
						...
						<img src="' . esc_url(admin_url('images/wpspin_light.gif')) . '" class="img-waiting" alt="" />
						<a href="javascript:;" class="button-secondary cancel js-dialog-close">'
						. __('Cancel', 'wpcf_access') . '</a>
						<a href="javascript:;" class="button-primary confirm js-confirm-remove"';
						if (!empty($users)) {
				...
				$output .= '</a>';
				$output .= '</div> <!-- .wpv-dialog-footer -->';
			$output .= '</div><!-- .wpv-dialog -->';
		$output .= '</div><!-- .wpcf-access-reassign-role -->';
	$output .= '</div><!-- .hidden -->';
    return $output;
}
