...
// parse form shortcode [credform]
    ...
    public function cred_field_shortcodes($atts) {
        ...
        if (!$taxonomy) {
            $fieldObj = null;
            if (
                    array_key_exists('post_fields', $this->out_['fields']) && is_array($this->out_['fields']['post_fields']) && in_array($field, array_keys($this->out_['fields']['post_fields']))
            ) {
                if ($post != $post_type)
                    return '';
            ...
            elseif (
                    array_key_exists('parents', $this->out_['fields']) && is_array($this->out_['fields']['parents']) && in_array($field, array_keys($this->out_['fields']['parents']))
            ) {
                ...
                /* $out='';
                  ...
                  return $out; */
                    ...
                    array_key_exists('form_fields', $this->out_['fields']) && is_array($this->out_['fields']['form_fields']) && in_array($field, array_keys($this->out_['fields']['form_fields']))
            ) {
                $name = $name_orig = $field;
